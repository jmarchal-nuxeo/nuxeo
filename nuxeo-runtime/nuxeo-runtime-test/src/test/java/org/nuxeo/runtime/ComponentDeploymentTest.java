...
public class ComponentDeploymentTest extends NXRuntimeTestCase {
    ...
    public void testStartupStatus() throws Exception {
        ...
        try {
            ...
            // it is error prone to assert hard coded values for . A change not impacting this test like removing a deprecated component will break the test
            // quick fix (use the real total registrations number)
            int totalContribs = mgr.getRegistrations().size();
            assertEquals(
                    "======================================================================\n"
                            + "= Component Loading Status: Pending: 1 / Missing: 1 / Unstarted: 0 / Total: "+totalContribs+"\n"
